...
var AnnotationSelector = Panel.extend({
    ...
    editAnnotationMetadata(evt) {
        ...
        const model = this.collection.get(id);
        this.listenToOnce(
            showSaveAnnotationDialog(model, {title: 'Chỉnh sửa chú thích', viewer: this.viewer}),
            'g:submit',
            () => {
                if (model.get('displayed')) {
    ...
    createAnnotation(evt) {
        ...
        this.listenToOnce(
            showSaveAnnotationDialog(model, {title: 'Tạo chú thích'}),
            'g:submit',
            () => {
    ...
    _changeGlobalOpacity() {
        ...
        this.$('.h-annotation-opacity-container')
            .attr('title', `Độ mờ đục tổng thể của chú thích ${(this._opacity * 100).toFixed()}%`);
    ...
    _changeGlobalFillOpacity() {
        ...
        this.$('.h-annotation-fill-opacity-container')
            .attr('title', `Độ mờ của chú thích ${(this._fillOpacity * 100).toFixed()}%`);
